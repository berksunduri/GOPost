...
GetStorageInfo:function(){return Promise.resolve({base_dir:"/Users/dev/api-workspace",format:"git-friendly (directory-per-collection)",schema:"1"})},
SetWorkspaceDir:function(p){return Promise.resolve({base_dir:p||"/Users/dev/api-workspace",format:"git-friendly (directory-per-collection)",schema:"1"})},
RevealWorkspace:function(){return Promise.resolve()},
BuildCICommand:function(c,e){return Promise.resolve('gopost run --data-dir "/Users/dev/api-workspace"'+(e?' --env "'+e+'"':'')+' --reporter junit --output results.xml "'+(c||'Petstore API')+'"')},
ExecuteRequestRaw:function(){return Promise.resolve(m.response)},
ExecuteGraphQLRequestRaw:function(){return Promise.resolve(m.response)},
